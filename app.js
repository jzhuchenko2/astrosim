...
function App() {
  const [separationStatus, setSeparationStatus] = useState("Pending");
  const [healthData, setHealthData] = useState({});

  // Fetch health data from the Flask API
  useEffect(() => {
    const interval = setInterval(() => {
      fetch("/health")
        .then(response => response.json())
  ...
  return (
    <div className="App">
      ...
      <ul>
        ...
        <li>Thrust Power: {healthData.thrust_power ? healthData.thrust_power.toFixed(2) : "Loading..."}%</li>
        <li>Battery Status: {healthData.battery_status || "Loading..."}</li>
        <li>Temperature: {healthData.temperature ? healthData.temperature.toFixed(2) : "Loading..."}°C</li>
      </ul>
    </div>
  );
}
